 ...
 * @LastEditTime: 2019-10-29 15:18:20
        ...
        <a-form :form="form" @submit="handleSubmitForm">
          ...
          <a-form-item
            ...
            <a-radio-group
              ...
              :disabled="!goodsDetail.is_type_changeable"
              v-decorator="[
                'type',
          ...
          <a-form-item
            v-if="goodsDetail.type === 2"
            label="兑换码清单"
            :labelCol="{lg: {span: 7}, sm: {span: 7}}"
            :wrapperCol="{lg: {span: 10}, sm: {span: 17} }">
            <a-button size="small" style="margin-right: 8px;" @click="handleDownloadUnused">下载未使用兑换码</a-button>
            <a-button type="primary" size="small" @click="handleShowUpload">上传</a-button>
          </a-form-item>
          <a-form-item
            label="商品库存"
            :labelCol="{lg: {span: 7}, sm: {span: 7}}"
          ...
          <a-form-item
            ...
            <span style="font-size: 10px; color: #999; margin-left: 8px;"><a-icon type="info-circle-o" />增加库存为正数，减少库存为负数</span>
          </a-form-item>
          <a-form-item
            label="价值积分"
            :labelCol="{lg: {span: 7}, sm: {span: 7}}"
          ...
          <a-form-item
            ...
            <a-button v-if="type === 1" htmlType="submit" type="primary">保存</a-button>
          </a-form-item>
        </a-form>
      </a-spin>
    </a-card>
    <a-modal :width="600" title="上传兑换码" v-model="visible" @ok="handleSubmitUploadFile">
      <a-spin :spinning="confirmLoading">
        <template>
...
<script>
import { addGoodCover, downloadGoodsCode, importGoodsCode, getGoodsDetail, editGoods, exportUnusedGoodsCode } from '@/api/goods'
...
export default {
  ...
  methods: {
    ...
    handleDownloadUnused () {
      exportUnusedGoodsCode(this.id).then(res => {
        window.location.href = res.url
        window.open(res.url)
      })
    },
